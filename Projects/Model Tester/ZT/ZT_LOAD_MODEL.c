        ...
        for (ii=0; ii<model->pol[i]->nbPolygon; ii++)
        {
            if (model->pol[i]->attbl[ii].dir & sprNoflip)
            {
                ...
                if (UseRealtimeGouraud==true)
                {
                    ...
                    {
                        ATTR bufAttr = ATTRIBUTE(model->pol[i]->attbl[ii].flag, model->pol[i]->attbl[ii].sort&0x03 , model->pol[i]->attbl[ii].texno, model->pol[i]->attbl[ii].colno, GRtbl(gouraudCounter), Window_In|MESHoff|HSSon|ECdis|SPdis|CL16Look|CL_Gouraud, sprNoflip, UseDepth|UseNearClip);
                        model->pol[i]->attbl[ii] = bufAttr;
                    }
                    else
                    {
                        ATTR bufAttr = ATTRIBUTE(model->pol[i]->attbl[ii].flag, model->pol[i]->attbl[ii].sort&0x03, model->pol[i]->attbl[ii].texno, model->pol[i]->attbl[ii].colno, GRtbl(gouraudCounter), Window_In|MESHoff|HSSon|ECdis|SPenb|CL16Look|CL_Gouraud, sprNoflip, UseDepth|UseNearClip);
                        model->pol[i]->attbl[ii] = bufAttr;
                    }
                    gouraudCounter++;
                }
            ...
            else
            {
                if (UseRealtimeGouraud==true)
                {
                    ATTR bufAttr = ATTRIBUTE(model->pol[i]->attbl[ii].flag, model->pol[i]->attbl[ii].sort&0x03, No_Texture, model->pol[i]->attbl[ii].colno, GRtbl(gouraudCounter), CL32KRGB|MESHoff|Window_In|CL_Gouraud, sprPolygon, UseDepth|UseNearClip);
                    model->pol[i]->attbl[ii] = bufAttr;
                    gouraudCounter++;
                }
            }
